...
RSpec.describe FactoryBot::ScopedSequence do
  ...
  describe "a scoped sequence with aliases using default value" do
    subject do
      FactoryBot::ScopedSequence.new(:test, :scope_attribute, aliases: %i[alias other]) do |n|
        "=#{n}"
      end
    end
    ...
    it "has the expected names as its names" do
      names = %i[foo bar baz]
      sequence = FactoryBot::ScopedSequence.new(names.first, :scope_attribute, aliases: names.last(2)) do
        "=#{n}"
      end
  ...
  describe "a scoped sequence with custom value and aliases" do
    subject do
      FactoryBot::ScopedSequence.new(:test, :scope_attribute, 3, aliases: %i[alias other]) do |n|
        "=#{n}"
      end
    end
    ...
    it "has the expected names as its names" do
      names = %i[foo bar baz]
      sequence = FactoryBot::ScopedSequence.new(names.first, :scope_attribute, 3, aliases: names.last(2)) do
        "=#{n}"
      end
      expect(sequence.names).to eq names
    end
